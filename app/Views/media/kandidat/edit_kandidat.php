...
<div class="content-wrapper">
    ...
    <section class="content-header">
        <h1>
            Kandidat
            <small>Ubah data kandidat</small>
        </h1>
        <ol class="breadcrumb">
        ...
        <div class="row tambah">
            ...
            <div class="col-md-8">
                ...
                <div class="box box-primary">
                    ...
                    <!-- /.box-header -->
                    <!-- form start -->
                    <form action="/admin/media_pemilihan/aksi_edit/<?= $media['slug']; ?>/kandidat/<?= $kandidat['kandidat_id']; ?>" method="post" enctype="multipart/form-data" id="kandidatForm">
                        <?= csrf_field(); ?>
                        ...
                        <div class="box-body">
                            ...
                            <div class="form-group <?= session('errors') && isset(session('errors')['nomor_urut']) ? 'has-error' : ''; ?>">
                                ...
                                <input type="number" class="form-control" id="nomor_urut" name="nomor_urut" value="<?= old('nomor_urut', $kandidat['nomor_urut']); ?>" placeholder="Masukkan Nomor Urut Kandidat">

                                <?php if (session('errors') && isset(session('errors')['nomor_urut'])) : ?>
                                    <span class="help-block"><?= esc(session('errors')['nomor_urut']); ?></span>
                                <?php endif; ?>
                            </div>
                            <!-- Start Pecah nama bila ada bila tidak -->
                            ...
                            <div class="form-group <?= session('errors') && isset(session('errors')['nama_kandidat']) ? 'has-error' : ''; ?>">
                                ...
                                <input type="text" class="form-control" id="nama_kandidat" name="nama_kandidat" value="<?= old('nama_kandidat', $pecahNamaKandidat[0]); ?>" placeholder="Masukkan Nama Ketua">
                            ...
                            <div class="checkbox">
                                <label>
                                    <input type="checkbox" id="wakil_kandidat" <?= count($pecahNamaKandidat) != 1 ? "checked" : "" ?>> Checklist ini untuk menambahkan <b>Wakil</b> <i>Ketua/Pemimpin</i> kandidat
                                </label>
                            </div>

                            <div id="form_wakil_kandidat" style="display: <?= count($pecahNamaKandidat) != 1 ? "block" : "none" ?>">
                                <!-- Nama wakil kandidat -->
                                <div class="form-group <?= session('errors') && isset(session('errors')['nama_wakil_kandidat']) ? 'has-error' : ''; ?>">
                                    ...
                                    <input type="text" class="form-control" id="nama_wakil_kandidat" name="nama_wakil_kandidat" value="<?= old('nama_wakil_kandidat', esc($wakil)) ?>" placeholder="Masukkan Nama Wakil Ketua">
                            ...
                            <div class="form-group <?= session('errors') && isset(session('errors')['poster']) ? 'has-error' : ''; ?>">
                                ...
                                <input type="text" name="oldPoster" value="<?= $kandidat['poster']; ?>" hidden>
                                ...
                                <img id="poster-preview" src="/img/kandidat/<?= $kandidat['poster']; ?>" alt="Pratinjau Poster">
                            </div>
